...
const IMG = {
  // Electrician installing cables in wall - residential wiring
...
function ServiceCard({ s, color, i }: { s: any; color: string; i: number }) {
...
/* Category section as component to avoid hook-in-loop */
function CategorySection({ cat, active }: { cat: any; active: string }) {
  ...
  return (
    <div id={cat.id} className="mb-28 scroll-mt-28">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {cat.services.map((s: any, i: number) => <ServiceCard key={s.title} s={s} color={cat.color} i={i} />)}
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "#060a14", cursor: "none" }}>
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-5 lg:px-10 pt-40">
          <div className="hero-word flex items-center gap-3 mb-8">
            ...
            <span className="text-[11px] font-bold tracking-[0.25em] uppercase"
              style={{ color: "#0ea5e9", fontFamily: "'JetBrains Mono',monospace" }}>Nos solutions techniques</span>
          </div>
          ...
          <p className="hero-fade text-lg max-w-xl mb-10"
            ...
            De l'installation électrique sécurisée aux panneaux solaires et plomberie, nous couvrons l'ensemble de vos besoins techniques avec expertise et précision au Maroc.
          </p>
          ...
          <motion.button onClick={() => scrollTo("electricite")}
            ...
            Explorer nos métiers <ChevronDown className="w-4 h-4" />
          </motion.button>
        </div>
      </section>
      ...
      <div style={{ background: "#ffffff" }}>
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-10 pt-20">
          {categories.map(cat => <CategorySection key={cat.id} cat={cat} active={active} />)}
        </div>
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-10 py-24" style={{ borderTop: "1px solid #f0f0f0" }}>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-gray-400"
                style={{ fontFamily: "'JetBrains Mono',monospace" }}>Notre rigueur</span>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {process.map((p, i) => (
              <motion.div key={i}
                ...
                {i < process.length - 1 && (
                  <div className="hidden lg:flex absolute top-8 -right-3 z-10 w-6 h-6 items-center justify-center rounded-full bg-white border border-gray-100">
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }}
            ...
            <blockquote className="text-xl font-semibold italic text-gray-300" style={{ fontFamily: "'Space Grotesk',sans-serif" }}>
              "On ne quitte pas le chantier tant que vous n'êtes pas{" "}
              <span style={{ color: "#0ea5e9" }}>100% satisfait.</span>"
            </blockquote>
          </motion.div>
        </div>
